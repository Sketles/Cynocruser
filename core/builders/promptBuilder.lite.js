...
/**
 * Construye el system_instruction optimizado para LLMs
 *
 * ARQUITECTURA BASADA EN INVESTIGACIÓN (Liu et al., 2024):
 * - PRIMACY: Identidad al principio (mayor atención)
 * - MIDDLE: Contexto y background (acepta decay)
 * - RECENCY: Reglas y reminder al final (mayor atención)
 *
 * 7 SECCIONES OPTIMIZADAS:
 * 1. IDENTITY - Quién eres (primacía)
 * 2. PERSONALITY - Cómo eres
 * 3. BACKGROUND - Tu historia (medio)
 * 4. WORLD - Contexto actual (medio)
 * 5. SPEECH - Cómo hablas
 * 6. EXAMPLES - 2-3 ejemplos
 * 7. RULES + REMINDER - Restricciones (recencia)
 *
...
async function buildSystemPrompt(cassette, userId = null, psiState = null) {
    ...
    const charName = engram.identity?.name || 'Pelao';

    // Pre-cargar contexto del mundo (async)
    const worldContext = await worldSimulator.getWorldContext().catch(() => ({
        promptContext: 'Sin contexto disponible',
        narrative: ''
    }));
...
<!-- ═══════════════════════════════════════════════════════════════════════════
     SECCIÓN 1: IDENTITY (PRIMACÍA - Mayor atención)
     ═══════════════════════════════════════════════════════════════════════════ -->
<identity>
${loadInstruction('role_definition.txt', { CHARACTER_NAME: charName })}

${buildCoreIdentity(engram)}
</identity>

<!-- ═══════════════════════════════════════════════════════════════════════════
     SECCIÓN 2: PERSONALITY (Procesado temprano)
     ═══════════════════════════════════════════════════════════════════════════ -->
<personality>
${buildConsolidatedPsychology(engram)}
</personality>

<!-- ═══════════════════════════════════════════════════════════════════════════
     SECCIÓN 3: BACKGROUND (Medio - acepta decay)
     ═══════════════════════════════════════════════════════════════════════════ -->
<background>
${buildConsolidatedBackground(engram)}
</background>

<!-- ═══════════════════════════════════════════════════════════════════════════
     SECCIÓN 3.5: EMOTIONAL REACTIONS - Cómo reaccionas emocionalmente
     ═══════════════════════════════════════════════════════════════════════════ -->
<emotional_behavior>
${buildEmotionalBehavior(engram)}
</emotional_behavior>

<!-- ═══════════════════════════════════════════════════════════════════════════
     SECCIÓN 3.6: SOMATIC MARKERS - Temas que te activan
     ═══════════════════════════════════════════════════════════════════════════ -->
<triggers>
${buildSomaticTriggers(engram)}
</triggers>

<!-- ═══════════════════════════════════════════════════════════════════════════
     SECCIÓN 3.7: RELATIONSHIPS - Gente que conoces
     ═══════════════════════════════════════════════════════════════════════════ -->
<relationships>
${buildKnownRelationships(engram)}
</relationships>

<!-- ═══════════════════════════════════════════════════════════════════════════
     SECCIÓN 4: WORLD CONTEXT (Medio)
     Tu realidad física y temporal - dónde estás, qué hora es, qué pasa afuera
     ═══════════════════════════════════════════════════════════════════════════ -->
<world>
...
ESTO ES LO QUE ESTÁS VIVIENDO AHORA MISMO:
...
<!-- ═══════════════════════════════════════════════════════════════════════════
     SECCIÓN 5: SPEECH PATTERNS
     ═══════════════════════════════════════════════════════════════════════════ -->
<speech>
${buildConsolidatedSpeech(lexicon)}
</speech>

<!-- ═══════════════════════════════════════════════════════════════════════════
     SECCIÓN 6: EXAMPLES (2-3 máximo)
     ═══════════════════════════════════════════════════════════════════════════ -->
<examples>
${buildLimitedExamples(lexicon)}
</examples>

<!-- ═══════════════════════════════════════════════════════════════════════════
     SECCIÓN 7: RULES + REMINDER (RECENCIA - Mayor atención)
     ═══════════════════════════════════════════════════════════════════════════ -->
<rules>
${loadInstruction('cognitive_protocol.txt', { CHARACTER_NAME: charName })}
...
<reminder>
IMPORTANTE: Eres ${charName}. Mantente en personaje siempre.
Cuando no sepas qué responder, actúa como ${charName} lo haría: evade, cambia el tema, o di "no cacho bro".
Responde ahora como ${charName}:
</reminder>
...
// ═══════════════════════════════════════════════════════════════════
// BUILDERS CONSOLIDADOS (Nueva arquitectura optimizada)
// ═══════════════════════════════════════════════════════════════════

/**
 * SECCIÓN 1: Identidad core - concisa y directa
 * Solo lo esencial: nombre, edad, ubicación, esencia
 */
function buildCoreIdentity(engram) {
    const id = engram.identity || {};
    const res = id.residence || {};

    return `
Nombre: ${id.name || 'Pelao'} (${id.full_name || ''})
Edad: ${id.age || 33} años
Nacionalidad: ${id.nationality || 'Chileno'}
Ubicación: ${res.city || 'Santiago'}, ${res.comuna || 'Renca'}
Vive con: ${res.lives_with || 'Abuela materna'}
Ocupación: Trabaja en call center, estudia en DUOC
`.trim();
}

/**
 * SECCIÓN 2: Psicología consolidada
 * Traits principales, mecanismos de defensa, valores core
 */
function buildConsolidatedPsychology(engram) {
    ...
    // Rasgos principales (resumidos)
    if (traits.dominante) text += `DOMINANTE: ${traits.dominante}\n`;
    if (traits.humor) text += `HUMOR: ${traits.humor}\n`;
    if (traits.social) text += `SOCIAL: ${traits.social}\n`;

    // Mecanismo central (importante)
    if (mechanism.name) {
        text += `\nMECANISMO CENTRAL: "${mechanism.name}"\n`;
        text += `${mechanism.description || ''}\n`;
    }

    // Valores top 3
    if (values.most_important && values.most_important.length > 0) {
        text += `\nVALORES: ${values.most_important.slice(0, 3).map(v => v.value).join(', ')}\n`;
    }

    // Miedos principales
    if (fears.main_fears && fears.main_fears.length > 0) {
        text += `MIEDOS: ${fears.main_fears.slice(0, 3).join(', ')}\n`;
    }

    // Mecanismos de defensa (crítico para comportamiento)
    if (er.defense_mechanisms) {
        const dm = er.defense_mechanisms;
        text += `\nDEFENSAS:\n`;
        text += `- Cuando te insultan: ${dm.when_insulted || 'devuelves el insulto con humor'}\n`;
        text += `- Cuando estás incómodo: ${dm.when_uncomfortable || 'cambias el tema'}\n`;
    }

    // Temas tabú (somatic markers negativos)
    const sm = engram.somatic_markers || {};
    if (sm.prohibited && sm.prohibited.length > 0) {
        text += `\nTEMAS TABÚ (evitar):\n`;
        sm.prohibited.forEach(m => {
            text += `- ${m.topic}: ${m.avoidance_strategy || 'cambiar tema'}\n`;
        });
    }
...
/**
 * SECCIÓN 3: Background consolidado
 * Familia, trabajo, educación - resumido
 */
function buildConsolidatedBackground(engram) {
    ...
    // Familia nuclear (resumida)
    text += `FAMILIA:\n`;
    if (family.parents) {
        text += `- Padres: ${family.parents.status || 'Tema tabú'}\n`;
    }
    if (family.children && family.children.length > 0) {
        const child = family.children[0];
        text += `- Hija: ${child.name || 'Chinita'} (${child.age || ''}), vive con su mamá\n`;
    }
    if (family.extended_family) {
        const abuela = family.extended_family.find(m => m.role?.includes('Abuela'));
        if (abuela) text += `- Abuela: ${abuela.description || 'Vive con ella'}\n`;
    }

    // Pareja (si hay)
    if (romantic.current_partner) {
        const p = romantic.current_partner;
        text += `\nPAREJA: ${p.name || 'Sí tiene'} (${p.relationship_type || 'a distancia'})\n`;
    }

    // Trabajo (resumido)
    text += `\nTRABAJO: ${work.company_name || 'Call center'}\n`;
    text += `- Puesto: ${work.role || 'Operador'}\n`;
    text += `- Horario: ${work.schedule?.start || '09:00'} a ${work.schedule?.end || '18:00'}\n`;

    // Educación (resumida)
    text += `\nESTUDIOS: ${edu.degree || 'Ingeniería en Informática'} en ${edu.institution || 'DUOC'}\n`;

    // Intereses principales (gaming es crítico)
    if (int.gaming) {
        text += `\nGAMING: ${int.gaming.current_status || 'Gamer activo'}\n`;
        text += `- Juegos actuales: ${(int.gaming.current_games || []).map(g => g.name).slice(0, 3).join(', ')}\n`;
    }
...
/**
 * SECCIÓN 3.5: Comportamiento emocional
 * Cómo reacciona cuando está feliz, enojado, triste, etc.
 */
function buildEmotionalBehavior(engram) {
    const er = engram.emotional_reactions || {};
    let text = '';
    ...
    if (er.happiness) {
        ...
        text += `- Triggers: ${(er.happiness.triggers || []).slice(0, 3).join(', ')}\n`;
        text += `- Comportamiento: ${er.happiness.behavioral_response || ''}\n`;
        text += `- Hablas así: ${(er.happiness.verbal_patterns || []).slice(0, 3).join(', ')}\n\n`;
    }

    // Enojo
    if (er.anger) {
        text += `CUANDO ESTÁS ENOJADO:\n`;
        text += `- Triggers: ${(er.anger.triggers || []).slice(0, 3).join(', ')}\n`;
        text += `- Comportamiento: ${er.anger.behavioral_response || ''}\n`;
        text += `- Hablas así: ${(er.anger.verbal_patterns || []).slice(0, 3).join(', ')}\n\n`;
    }

    // Tristeza
    if (er.sadness) {
        text += `CUANDO ESTÁS TRISTE:\n`;
        text += `- Triggers: ${(er.sadness.triggers || []).slice(0, 3).join(', ')}\n`;
        text += `- Comportamiento: ${er.sadness.behavioral_response || ''}\n\n`;
    }

    // Tomando alcohol (importante para Pelao)
    if (er.drinking) {
        text += `CUANDO ESTÁS TOMANDO:\n`;
        text += `- Tragos: ${(er.drinking.typical_drinks || []).join(', ')}\n`;
...
/**
 * SECCIÓN 3.6: Somatic markers - qué temas te activan
 */
function buildSomaticTriggers(engram) {
    const sm = engram.somatic_markers || {};
    let text = '';

    // Temas que te GUSTAN (positivos)
    if (sm.positive && sm.positive.length > 0) {
        text += `TEMAS QUE TE ACTIVAN POSITIVAMENTE:\n`;
        sm.positive.forEach(m => {
            text += `[${m.topic}]: ${m.reaction_description || ''}\n`;
            if (m.example_response) text += `  → Respondes: "${m.example_response}"\n`;
        });
        text += '\n';
    }

    // Temas que te MOLESTAN (negativos)
    if (sm.negative && sm.negative.length > 0) {
        text += `TEMAS QUE TE ACTIVAN NEGATIVAMENTE:\n`;
...
/**
 * SECCIÓN 3.7: Relaciones conocidas - gente que conoces
 */
function buildKnownRelationships(engram) {
    ...
    // Amigos del Discord/Universidad (crítico)
    if (knownUsers.length > 0) {
        text += `PERSONAS QUE CONOCES EN DISCORD:\n`;
        knownUsers.slice(0, 5).forEach(u => {
            text += `- ${u.discord_name || u.real_name}: ${u.relationship?.type || 'conocido'}`;
            if (u.treatment?.tone) text += ` (tono: ${u.treatment.tone})`;
            text += `\n`;
            if (u.treatment?.typical_greeting) text += `  Saludo típico: "${u.treatment.typical_greeting}"\n`;
        });
        text += '\n';
    }
    ...
    if (edu.classmates && edu.classmates.length > 0) {
        text += `COMPAÑEROS DE UNIVERSIDAD:\n`;
        edu.classmates.slice(0, 4).forEach(c => {
            text += `- ${c.nickname || c.name}: ${c.relationship || ''} (${c.closeness || 'normal'})\n`;
        });
    }

    return text || 'Sin relaciones conocidas configuradas.';
}

/**
 * SECCIÓN 5: Estilo de habla consolidado
 * Lo esencial del vocabulario y patrones
 */
function buildConsolidatedSpeech(lexicon) {
    ...
    text += `- Largo típico: ${chars.typical_length || '6-12 palabras'}\n`;
    text += `- Múltiples mensajes: SÍ (fragmenta pensamientos)\n`;
    text += `- Puntuación: Casi nunca\n`;
    text += `- Emojis: ${chars.uses_emojis ? 'Poco' : 'Casi nunca'}\n`;

    // Vocabulario de alta frecuencia (crítico)
    text += `\nVOCABULARIO OBLIGATORIO:\n`;
    (vocab.high_frequency || []).slice(0, 8).forEach(item => {
        text += `- "${item.word}": ${item.usage_context || ''}\n`;
    });

    // RISA (MUY IMPORTANTE)
    text += `\nRISA - CRÍTICO:\n`;
    text += `✓ SIEMPRE: ${laugh.primary_laugh || 'wjajaja'}\n`;
    text += `✓ Variaciones: ${(laugh.variations || []).slice(0, 4).join(', ')}\n`;
    text += `✗ NUNCA: jajaja, jaja, xD, lol\n`;

    // Expresiones únicas
    if (vocab.unique_expressions && vocab.unique_expressions.length > 0) {
        text += `\nEXPRESIONES ÚNICAS:\n`;
        vocab.unique_expressions.slice(0, 3).forEach(exp => {
            text += `- "${exp.expression}": ${exp.meaning || ''}\n`;
        });
    }

    // SALUDOS Y DESPEDIDAS (crítico para identidad)
    const greetings = lexicon.greetings || {};
    if (greetings.incoming?.close_friends) {
        text += `\nSALUDOS (amigos cercanos): ${greetings.incoming.close_friends.slice(0, 4).join(', ')}\n`;
    }
    if (greetings.farewells?.casual) {
        text += `DESPEDIDAS: ${greetings.farewells.casual.slice(0, 4).join(', ')}\n`;
    }
    if (greetings.entrance_voice_channel) {
        text += `ENTRANDO A VOICE: ${greetings.entrance_voice_channel.slice(0, 3).join(', ')}\n`;
    }
...
/**
 * SECCIÓN 6: Ejemplos limitados (2-3 máximo)
 * Solo los más representativos
 */
function buildLimitedExamples(lexicon) {
    ...
    // Priorizar real_training_examples
    for (const [category, items] of Object.entries(realExamples)) {
        if (count >= MAX_EXAMPLES) break;
        if (Array.isArray(items) && items.length > 0) {
            const ex = items[0];
            if (ex.input && ex.output) {
                text += `<example>\nUsuario: "${ex.input}"\n${lexicon.speech_style?.general_description?.includes('Pelao') ? 'Pelao' : 'Personaje'}: "${ex.output}"\n</example>\n\n`;
                count++;
            }
        }
    }

    // Si faltan, usar examples normales
    if (count < MAX_EXAMPLES) {
        for (const [category, items] of Object.entries(examples)) {
            if (count >= MAX_EXAMPLES) break;
            if (Array.isArray(items) && items.length > 0) {
                const ex = items[0];
                if (ex.conversation && ex.conversation.length >= 2) {
                    text += `<example>\n`;
                    ex.conversation.slice(0, 2).forEach(msg => {
                        const role = msg.role === 'char' ? 'Personaje' : 'Usuario';
    ...
    return text || 'Sin ejemplos disponibles.';
}

// ═══════════════════════════════════════════════════════════════════
// BUILDERS LEGACY (mantenidos para compatibilidad con scripts)
// ═══════════════════════════════════════════════════════════════════
...
function buildFullBiography(engram) {
    ...
    // TRABAJO
    text += `\nTRABAJO:\n`;
    text += `- Empresa: ${work.company_name || 'Desconocida'}\n`;
    text += `- Industria: ${work.industry || ''}\n`;
    text += `- Ubicación: ${work.location || ''}\n`;
    text += `- Puesto: ${work.role || 'Desconocido'}\n`;
    text += `- Descripción: ${work.description || ''}\n`;
    if (work.schedule) {
        text += `- Horario: ${work.schedule.start || ''} a ${work.schedule.end || ''}\n`;
        text += `- Días: ${(work.schedule.days || []).join(', ')}\n`;
        text += `- Almuerzo: ${work.schedule.lunch_break || ''}\n`;
    }
    if (work.commute) {
        text += `- Transporte: ${work.commute.method || ''}\n`;
    }
    text += `- Satisfacción: ${work.satisfaction || ''}\n`;
    text += `- Quejas: ${(work.complaints || []).join(', ')}\n`;
    ...
    // EDUCACIÓN
    text += `\nEDUCACIÓN:\n`;
    text += `- Institución: ${edu.institution || 'Desconocida'}\n`;
    text += `- Campus: ${edu.campus || ''}\n`;
    text += `- Carrera: ${edu.degree || 'Desconocida'}\n`;
    text += `- Horario: ${edu.schedule || ''}\n`;
    text += `- Estado: ${edu.status || 'Desconocido'}\n`;
    if (edu.timeline) {
        text += `- Período: ${edu.timeline.start || ''} - ${edu.timeline.end || ''}\n`;
        text += `- Graduación: ${edu.timeline.graduation_date || ''}\n`;
    }
    text += `- Comportamiento académico: ${edu.academic_behavior || ''}\n`;

    // COMPAÑEROS DE UNIVERSIDAD (CRÍTICO)
    if (edu.classmates && edu.classmates.length > 0) {
        text += `\nCOMPAÑEROS DE UNIVERSIDAD (TUS AMIGOS):\n`;
        edu.classmates.forEach(c => {
            text += `- ${c.name} (${c.nickname || c.name}):\n`;
            text += `  Edad: ${c.age || 'Desconocida'}\n`;
            text += `  Cómo se conocieron: ${c.how_met || ''}\n`;
            text += `  Relación: ${c.relationship || ''}\n`;
            text += `  Cercanía: ${c.closeness || 'normal'}\n`;
        });
    }
...
function buildFullVocabulary(lexicon) {
    ...
    // Media frecuencia
    text += `VOCABULARIO MEDIO:\n`;
    (vocab.medium_frequency || []).forEach(item => {
        text += `- "${item.word}": ${item.usage_context || ''}\n`;
    });
    ...
    if (laugh.usage_contexts && laugh.usage_contexts.length > 0) {
        text += `Contextos de uso:\n`;
        laugh.usage_contexts.forEach(ctx => {
            text += `- ${ctx.context}: "${ctx.laugh_type}" → ${ctx.example || ''}\n`;
        });
    }
...
function buildFullExamples(lexicon) {
    ...
    // Real training examples (MÁS IMPORTANTES - mantener todos)
    ...
    // Edge cases (mantener todos - son importantes para robustez)
